...
{
    if (stepperState == offs) return ERROR;
...
{
    int8_t result = ERROR;
    if ((stepperState == offs) || (steps == 0)) return ERROR;
...
{
    if ((stepperState == offs) || (stepperState == halted)) return ERROR;
    stepperState = halted;
    return SUCCESS;
}
