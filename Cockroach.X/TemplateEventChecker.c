...
uint8_t CheckBumpers(void){
    static ES_EventTyp_t lastEvent = BUMPER_NOT_TRIGGERED;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t bumpVal = Roach_ReadBumpers(); // read the bumper 

    if (bumpVal) { // is battery connected?
        printf("registered bump\n");
        curEvent = BUMPER_TRIGGERED;
    } else {
        //printf("not registered bump\n");
        curEvent = BUMPER_NOT_TRIGGERED;
    }
    if (curEvent != lastEvent) { // check for change from last time
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = bumpVal;
        returnVal = TRUE;
        lastEvent = curEvent; // update history
        //PostTemplateFSM(thisEvent);
        PostTemplateHSM(thisEvent);
#ifndef EVENTCHECKER_TEST           // keep this as is for test harness
        //printf("\r\nFront Right Bumper Triggered! "); // Change it to your target service's post function
#else
        SaveEvent(thisEvent);
#endif   
    }
...
uint8_t CheckLightToDark(void){
    static ES_EventTyp_t lastEvent = DARK_TO_LIGHT;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t darkVal = Roach_LightLevel(); // read the bumper 

    if (darkVal > LIGHT_THRESHOLD) { // 
        //printf("registered light\n");
        curEvent = LIGHT_TO_DARK;
    } else {
        //printf("not registered light\n");
        curEvent = DARK_TO_LIGHT;
    }
    if (curEvent != lastEvent) { // check for change from last time
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = darkVal;
        returnVal = TRUE;
        lastEvent = curEvent; // update history
        //PostTemplateFSM(thisEvent);
        PostTemplateHSM(thisEvent);
...
uint8_t CheckDarkToLight(void){
    static ES_EventTyp_t lastEvent = LIGHT_TO_DARK;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    uint16_t lightVal = Roach_LightLevel(); // read the bumper 
