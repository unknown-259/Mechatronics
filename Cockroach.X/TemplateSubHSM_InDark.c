...
#include "TemplateSubHSM_InDark.h"
#include <stdio.h>
...
static TemplateSubHSMInDarkState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
uint8_t InitTemplateSubHSM_InDark(void)
{
    ...
    returnEvent = RunTemplateSubHSM_InDark(INIT_EVENT);
...
ES_Event RunTemplateSubHSM_InDark(ES_Event ThisEvent)
{
    uint8_t makeTransition = FALSE; // use to flag transition
    TemplateSubHSMInDarkState_t nextState; // <- change type to correct enum
    ...
    case Idle: // in the first state, replace this with correct names
        ...
        if ((ThisEvent.EventType == BUMPER_TRIGGERED)) {
            if(ThisEvent.EventParam == FLEFT_BUMP_MASK){
                ...
                ES_Timer_InitTimer(backup_2, 0.5 * 1000);  // Start timer
                nextState = Backwards;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            }
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunTemplateSubHSM_InDark(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunTemplateSubHSM_InDark(ENTRY_EVENT); // <- rename to your own Run function
    }
